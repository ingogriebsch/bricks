...
public class PolylineBuilderTest {
    ...
    public void builder_should_throw_exception_if_input_is_null() {
        assertThrows(NullPointerException.class, () -> PolylineBuilder.builder(null));
    }

    @Test
    public void build_should_return_builder() {
        assertThat(PolylineBuilder.builder(document)).isNotNull();
    }

    @Test
    public void build_should_return_matching_element() {
        PolylineBuilder builder = PolylineBuilder.builder(document);
        assertThat(builder.build()).isNotNull().isInstanceOf(SVGPolylineElement.class);
    }

    @Test
    public void points_should_throw_exception_if_input_is_null() {
        assertThrows(NullPointerException.class, () -> PolylineBuilder.builder(document).points((Point[]) null));
    }

    @Test
    public void build_should_return_element_containing_matching_attributes() {
        Point point = new Point(10, 10);
        Element element = PolylineBuilder.builder(document).points(point).build();
